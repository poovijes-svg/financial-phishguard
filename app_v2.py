...
class SimplePhishingDetector:
    def predict(self, features):
        ...
        # URL length check (phishing URLs are often longer)
        if features.get('url_length', 0) > 75:
            score += 1

        # IP address check (using IP instead of domain is suspicious)
        if features.get('has_ip', 0) == 1:
            score += 2

        # Special characters check (many special chars are suspicious)
        if features.get('num_special_chars', 0) > 5:
            score += 1

        # Simple threshold-based decision
        return 1 if score >= 2 else 0  # 1 = phishing, 0 = legitimate
